...
export default class DCMotor {
     ...
     */
    static makeTransmission(motor: DCMotor, num_motors: number, gear_reduction: number, efficiency: number) {
        return new DCMotor(
            num_motors * gear_reduction * efficiency * motor.m_kt,
    ...
    private m_kt: number;
    private m_kv: number;
    private m_resistance: number;
    private m_motor_inertia: number;
     ...
     *            (kg*m^2)
     */
    constructor(kt: number, kv: number, resistance: number, inertia: number) {
        this.m_kt = kt;
        this.m_kv = kv;
     ...
     * @param current
     */
    reset(position: number, velocity: number, current: number) {
     ...
     */
    step(applied_voltage: number, load: number, external_torque: number, timestep: number) {
        /*
        ...
        let acceleration =
            ((applied_voltage - this.m_velocity / this.m_kv) * this.m_kt) / (this.m_resistance * load) +
